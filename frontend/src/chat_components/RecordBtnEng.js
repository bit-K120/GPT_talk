...
const RecordBtnEng = ({onAppendMessage}) => {
...
// サーバーの定義
  ...
  useEffect(() => {
    ...
    newSocket.on("response_to_react", (data) => {
      ...
      console.log("受け取った音声データ:",data.text_to_speech_data)
      processData(data); 
      playAiVoice(data)
    });
...
// 音声取り込みの関数
  const startListening = () => SpeechRecognition.startListening({continuous:true, language:"en-AU"});
